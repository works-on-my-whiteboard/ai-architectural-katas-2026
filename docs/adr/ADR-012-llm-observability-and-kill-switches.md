...
The judges ask how the team will know when AI-driven functionality starts misbehaving in production. Generative output drifts with model updates, prompt edits, content changes and adversarial guests. Observability is a ranked characteristic of the base architecture, and every device and service is already traced; model calls must be traced to the same standard.
...
Guardrails run in two places, and the split matters. Content safety, denied topics and PII redaction are a managed guardrail policy in the catalogue, named per capability and applied on the way in and on the way out; the estate configures it but does not implement it. Schema validation, citation coverage and the capability's own refusal rules are asserted by the model access layer after the response returns, because they are specific to the feature and have to hold for Tier 2 and Tier 3 answers too. Outcomes from both are traced identically, so a guardrail regression looks the same whichever side produced it.
