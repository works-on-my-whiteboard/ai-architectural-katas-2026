...
The brief provides a budget for MQTT-capable devices. Gate scanners, people counters, enclosure sensor kits, smart feeders, cameras and ride sensors will all publish over MQTT. The park's WiFi is patchy and the backhaul to the cloud is not guaranteed. Sensor data feeds both safety alarms (which must fire locally) and analytics (which can tolerate delay).

Forces:

- Alarms about water chemistry or a ride vibration spike must reach keepers and ops without the cloud.
...
Alarm rules run on the gateway against the local broker and publish to `vde/{site}/{zone}/alarm/...`, which keeper tablets subscribe to over the zone network.
...
- Local fan-out means alarms are independent of the cloud.
...
| Latency | Local alarms in milliseconds; cloud analytics delayed during outages | Alarm rules on the gateway |
